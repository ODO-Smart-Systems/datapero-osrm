...
sql_con = assert( sql_env:connect('gis', 'datapero', 'DataPer0', "localhost", 5434) ) -- you can add db user/password here if needed
...
function process_way(profile, way, result)
  ...
  local handlers = Sequence {
    ...
    WayHandlers.weighted_bars
  }

  WayHandlers.run(profile, way, result, data, handlers)
end

function WayHandlers.weighted_bars(profile,way,result,data)
 ...
 local sql_query = " SELECT " ..
   "(SELECT COUNT(type) AS amenities_count " .. 
   " FROM osm_new_amenities " ..
   " WHERE ST_DWithin( (SELECT geometry FROM osm_new_mainroads WHERE osm_id=" .. way:id() .. ") ,osm_new_amenities.geometry,75) AND " ..
   "  (osm_new_amenities.type='bar' OR osm_new_amenities.type='cafe'))," ..
   "(SELECT COUNT(type) AS landusages_count " .. 
   " FROM osm_new_landusages " ..
   " WHERE ST_DWithin( (SELECT geometry FROM osm_new_mainroads WHERE osm_id=" .. way:id() .. ") ,osm_new_landusages.geometry,75) AND " ..
